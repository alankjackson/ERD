...
Summary_table <- FreqFliers %>% 
  ...
  select(censusBlockGroupFips, Label, Pop_blk_grpE,
         ...
         # Num_Claims_Own, Num_Claims_Rent, HouseholdsE, AvgHouseIncome) %>% 
  gt() %>%     
  gt::tab_header(
    ...
    tab_spanner(
      ...
      # columns=c(HouseholdsE, AvgHouseIncome)
    ) %>% 
    tab_footnote(
...
i = FreqFliers$censusBlockGroupFips[1]
  ...
  Claims %>% ggplot(aes(x=yearOfLoss, y=Claims)) +
   ...
   ggsave("EventsPerYr.png", width=3.5, height=3, units="in")
...
MyTable %>% 
  ...
  FloodInches <- Frequent %>% 
   ...
   ggsave("FloodInches.png", width=3.5, height=3, units="in")
...
box <- TempMap %>%
 ...
 BlkMap <-
   ...
   ggsave("BlkMap.png", width=7, units="in")

#   Build dataframe of plots

df <- dplyr::tribble(~label, ~map, ~table, ~floodbar, ~yearbar,
              "Harris A", "BlkMap.png", "Table.png", 
              "FloodInches.png", "EventsPerYr.png")

# #| echo = FALSE, results = "asis"
# cat("\n\n\\pagebreak\n")
# cat("##Plots From Harris 1")
# #| layout-ncol: 2
# #| fig-cap:
# #| - "Map of Block Group"
# #| - "Significant Flood Events"
# knitr::include_graphics("BlkMap.png")
...
res <- pmap_chr(df, \(label, map, table, floodbar, yearbar) {
  knitr::knit_child(text = c(
    "\\newpage",
    "## `r label`",
    "::: {#fig-Map layout-ncol=1}",
    "![Map](`r map`){#fig-map}",
    ":::",
    "::: {#fig-Table layout-ncol=1}",
    "![Summary](Table.png){#fig-summary}",
    ":::",
    "::: {#fig-BarCharts layout-ncol=2}",
    # "![Inches water](FloodInches.png){#fig-flood}",
    "![Events per Year](EventsPerYr.png){#fig-evnts}",
    "This is label for the plots",
    ":::",
