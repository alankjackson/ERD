  ...
  Stats_by_block <- val_by_block2 %>% 
    group_by(GEOID) %>% 
      summarize(Mean=mean(value, na.rm=TRUE),
                Wmean=stats::weighted.mean(value, coverage_fraction, na.rm=TRUE),
...
All_stats <- readRDS(paste0(bigpath, "All_stats_", State,".rds"))
...
foo <- All_stats2 %>% 
  ...
  group_by(GEOID, Depth) %>% 
    summarise(NAME=first(NAME),
              ...
              Weights=sum(Weights),
              Fam_house=first(Fam_house),
              NonFam_house=first(NonFam_house),
              NumNA=sum(NumNA),
              Num=sum(Num)) %>% 
  ungroup()
...
BG_stats <- foo %>% 
  group_by(Blk_grp, Depth) %>% 
    summarise(T_pop=sum(Pop),
              ...
              T_NumNA=sum(NumNA),
              Num_blks=n(),
              Mean=sum(Wvaluesum)/sum(Weights)
              )
...
acs_vars <- c(Med_inc="B19013_001", # median household income, blk grp
...
ACS <- ACS %>% 
  ...
  select(GEOID, Med_inc=Med_incE, Per_cap_in=Per_cap_incE,
...
All_BG <- All_BG %>% 
  left_join(., ACS, by=join_by(Blk_grp==GEOID), 
                  relationship="many-to-one") %>% 
  relocate(T_pop, .after=Pop) %>% 
  relocate(Depth, .after=T_pop)
