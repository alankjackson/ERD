...
infiles <- list.files(path=bigpath, pattern="^FL_*") 
...
for (file in infiles) {
  ...
  print(paste(file, "------", length(infiles)-i, "Time remaining",
        length(infiles)*(tsum/i)/60, "minutes"))
  i <-  i + 1
...
for (file in infiles) {
  ...
  All_stats_FL <- rbind(All_stats_FL, Block_stats)
  
  #   just in case
  
  saveRDS(All_stats_FL, paste0(bigpath, "All_stats_FL.rds"))
}
```
...
tmap::tm_shape(All_stats2 %>% filter(GEOID=="220070503003030")) + 
