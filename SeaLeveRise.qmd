...
Each state conveniently includes a text file of the counties included, 
...
All_stats2 <- All_stats %>% 
  rename(Pop_blk=Pop, Fam_house_blk=Fam_house,
         NonFam_house_blk=NonFam_house) %>% 
...
Block_data <- All_BG %>% 
  select(GEOID, Depth, NAME, Pop_blk, Wmean, Fam_house_blk,
         NonFam_house_blk, Blk_grp, Pct_Pop_at_risk=Pop_pct, 
         Flood_pct)  
  #   Add geometry
...
All_BG2 <- All_BG2 %>% 
  ...
  mutate(Pct_pop_at_risk=Pop_at_risk/Pop_acs)
...
All_BG2 %>% 
  mutate(Depth=as.numeric(Depth)) %>% 
  mutate(Pop_fraction=T_pop/Pop_acs) %>% #fraction of pop in flooding blocks
  filter(Depth>9.6) %>% 
...
All_BG2 %>% 
  mutate(Depth=as.numeric(Depth)) %>% 
  ggplot(aes(x=Flood_pct*100, y=Pop_blk)) +
  geom_point() +
  facet_wrap(~ Depth, nrow=5) +
  labs(title="Flooding in Alabama at different levels of sea level rise",
       x="Percent of Census Block flooded",
...
All_BG2 %>% 
  mutate(Depth=as.numeric(Depth)) %>% 
  ggplot(aes(x=Flood_pct*100, y=Pct_poverty)) +
  geom_point() +
  facet_wrap(~ Depth, nrow=5) +
  labs(title="Flooding in Alabama at different levels of sea level rise",
       x="Percent of Census Block flooded",
...
foo3 <- terra::crop(foo3, Blocks_dhc)
...
#   tm_raster(alpha = 0.3)
# ) 
  tmap::tm_shape(Blocks_dhc) +
    tmap::tm_polygons(alpha=0.0)
```
