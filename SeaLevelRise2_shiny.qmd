...
dataset_grp <- reactive({
  ...
  # print(paste("dataset_grp:", foo %>% filter(Blk_grp=="010030109071")))
  foo
})

dataset_blk <- reactive({
  foo <- df_blk() %>% filter(Depth==input$Depth) %>%
    filter(GEOID %in% unlist(dataset_grp()[["Blocks"]]))
  # print(paste(foo$GEOID, foo$Blk_grp))
  # foo2 <- df_blk() %>% filter(Blk_grp=="010030109071") %>% sf::st_drop_geometry()
  # foo3 <- foo2 %>% filter(Depth==input$Depth)
  # foo4 <- foo3 %>% filter(GEOID %in% unlist(dataset_grp()[["Blocks"]]))
  # print(paste("dataset_blk 1:", foo2))
  # print(paste("dataset_blk 2:", foo3))
  # print(paste("dataset_blk 3:", foo4))
  foo
})
...
output$map <- leaflet::renderLeaflet({
  ...
  print(paste("Grp_data:", Grp_data %>% filter(Blk_grp=="010030109071")))
