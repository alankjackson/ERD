...
df_grp <- reactive({
  ...
  #     arrange(desc(Mean_depth))
  # Initial_select <- Initial_select[1,]$Blk_grp
  # Max_poverty <<- 5*round((max(foo$Pct_poverty, na.rm=TRUE)+2.5)/5)
  # print(paste("--0--", Max_poverty, input$Pct_poverty))
  # keep_tif <<- input$State # reset selected tile to none
  keep_tif <<- "None"
  foo
  }
})
...
observe({
  ...
  print(paste("tif stuff:", tif(), Tiff_path, length(tif())))
